...
pub fn build_api_router(state: AppState) -> Router {
    ...
    let authenticated = Router::new()
        ...
        .route(
            "/admin/certstore/:name/fullchain",
            get(api::get_certstore_fullchain),
        )
        ...
        .route("/admin/corgis/:id/hooks", get(api::get_corgi_hooks))
    ...
    let bootstrap_routes = Router::new()
        .route(
            "/bootstrap/admin-cert",
            post(bootstrap::bootstrap_admin_cert),
        )
...
pub async fn run(
    ...
    let agent_listener = bind_tcp(&config.bind, config.agent_port)
        ...
        .with_context(|| {
            format!(
                "Binding agent server on {}:{}",
                config.bind, config.agent_port
            )
        })?;
    ...
    tracing::info!(
        ...
        "API router built (code=S, has /flock)"
    );
