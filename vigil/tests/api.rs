...
async fn health_returns_status_and_ca_info() {
    ...
    assert!(body["ca"]["initialized"].as_bool().unwrap_or(false), "CA must be initialized");
    assert!(body["ca"]["fingerprint256"].is_string(), "CA fingerprint must be present");
    assert!(body["certificates"]["total"].is_number(), "cert stats must be present");
}
...
async fn ca_info_has_fingerprint_and_subject() {
    ...
    let body: Value = resp.json().await.unwrap();
    let ca = &body["rootCA"];
    assert!(ca["fingerprint256"].is_string(), "fingerprint256 must be present");
    ...
    assert!(subject.contains("Credo Test Intermediate CA"),
...
async fn sign_cert_and_fetch_by_id() {
    ...
    let cert_pem = sign_body["certificate"]["certPem"].as_str().expect("certPem present");
    assert!(cert_pem.contains("BEGIN CERTIFICATE"), "certPem must be valid PEM");

    // Fetch by ID
    let get_resp = vigil.client
        .get(format!("{}/certificates/{}", vigil.url, cert_id))
        .send().await.unwrap();

    assert_eq!(get_resp.status(), 200);
    let get_body: Value = get_resp.json().await.unwrap();
    assert!(!get_body["certificate"]["revoked"].as_bool().unwrap_or(true),
...
async fn revoke_cert_marks_revoked_in_db() {
    ...
    assert!(rev["certificate"]["revoked"].as_bool().unwrap_or(false),
        "certificate must be marked revoked");

    // GET /certificates/:id must now show revoked: true
    let get: Value = vigil.client
        .get(format!("{}/certificates/{}", vigil.url, cert_id))
        .send().await.unwrap()
        .json().await.unwrap();
...
async fn ocsp_status_transitions_with_revocation() {
    ...
    let after: Value = vigil.client
        .get(format!("{}/ocsp/{}", vigil.url, cert_id))
        .send().await.unwrap()
        .json().await.unwrap();
    assert_eq!(after["ocsp"]["status"].as_str(), Some("revoked"),
...
async fn crl_json_lists_revoked_certs() {
    ...
    let body: Value = resp.json().await.unwrap();
    let revoked = body["crl"]["revokedCertificates"].as_array().expect("revokedCertificates array");
    assert_eq!(revoked.len(), 1, "CRL must contain exactly the one revoked cert");
...
async fn crl_pem_endpoint_returns_valid_format() {
    ...
    let ct = resp.headers()
        ...
        .unwrap_or("");
    assert!(ct.contains("pem") || ct.contains("text") || ct.contains("octet"),
        "content-type should indicate PEM");

    let body = resp.text().await.unwrap();
    assert!(body.contains("BEGIN X509 CRL"), "body must contain PEM CRL header");
    assert!(body.contains("END X509 CRL"),   "body must contain PEM CRL footer");
}
...
async fn crl_der_endpoint_returns_binary() {
    ...
    let ct = resp.headers()
        ...
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    assert!(ct.contains("pkix-crl") || ct.contains("octet"),
        "content-type must indicate DER CRL, got: {ct}");

    let bytes = resp.bytes().await.unwrap();
    assert!(!bytes.is_empty(), "DER response must not be empty");
    assert_eq!(bytes[0], 0x30, "DER CRL must start with SEQUENCE tag (0x30)");
}
