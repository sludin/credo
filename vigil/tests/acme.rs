...
async fn acme_directory_has_required_fields() {
    ...
    assert!(
        ...
        "newAccount URL must be present"
    );
    assert!(body["newOrder"].is_string(), "newOrder URL must be present");
    assert!(body["newNonce"].is_string(), "newNonce URL must be present");
    assert!(
        body["revokeCert"].is_string(),
        "revokeCert URL must be present"
    );
    assert!(
        body["keyChange"].is_string(),
        "keyChange URL must be present"
    );
}
...
async fn new_nonce_is_unique_per_call() {
    ...
    for _ in 0..5 {
        ...
        assert!(
            ...
            "nonce must be unique, got duplicate: {nonce}"
        );
    }
    ...
    assert!(
        nonces.insert(head_nonce),
        "HEAD nonce must be unique from GET nonces"
    );
}
...
async fn acme_directory_urls_share_host_with_server() {
    ...
    for field in [
        ...
        assert!(
            ...
            "{field} must be an absolute URL, got: {url}"
        );
    }
}
