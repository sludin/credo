...
fn fixtures() -> PathBuf {
    ...
    PathBuf::from(manifest).parent().unwrap().join("tests/fixtures")
}
...
fn test_ca_metadata() -> RootCAMetadata {
    use vigil::config::{CaConfig, IssuancePolicyConfig, TlsConfig, VigilConfig, LogLevel};
    ...
    let config = VigilConfig {
        ...
        ca: CaConfig { curve: "P-256".to_string(), cert_default_days: 365, crl_next_update_hours: 24, ocsp_max_age_seconds: 60 },
        ...
        tls: TlsConfig { key_path: tmp.path().join("k"), cert_path: tmp.path().join("c"), client_ca_path: f.join("catrust.pem") },
        log_level: LogLevel::Warn,
        rbac_identities: vec![],
        issuance_policy: IssuancePolicyConfig {
            allowed_dns_suffixes: vec!["credo.test".to_string()],
            allow_subdomains: true, allow_bare_suffix: true,
...
fn ocsp_good_status_for_active_cert() {
    ...
    let _rec = storage::issue_certificate_record(&db, &store, dummy_record("c1", "ff10"), DUMMY_PEM).unwrap();

    let by_id = get_ocsp_status_by_cert_id(&db, "c1", 60).unwrap();
    assert_eq!(by_id.status, "good");
    assert!(by_id.next_update > by_id.this_update, "nextUpdate must be after thisUpdate");
...
fn ocsp_revoked_status_after_revocation() {
    ...
    storage::revoke_certificate(&db, "c2", "admin", "superseded", Some("admin".to_string()), None, Some("api".to_string())).unwrap();
...
fn crl_contains_only_revoked_certs() {
    ...
    storage::issue_certificate_record(&db, &store, dummy_record("active", "0001"), DUMMY_PEM).unwrap();
    storage::issue_certificate_record(&db, &store, dummy_record("revoked", "0002"), DUMMY_PEM).unwrap();
    storage::revoke_certificate(&db, "revoked", "admin", "keyCompromise", None, None, None).unwrap();
...
fn crl_is_empty_when_nothing_revoked() {
    ...
    assert!(crl.revoked_certificates.is_empty(), "CRL must be empty when nothing is revoked");
}
