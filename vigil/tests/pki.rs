...
fn signed_cert_preserves_dns_sans() {
    ...
    let dns_sans: Vec<&str> = cert
        .subject_alternative_name().ok().flatten()
        .map(|san| {
            san.value.general_names.iter().filter_map(|n| {
                if let x509_parser::extensions::GeneralName::DNSName(d) = n { Some(*d) } else { None }
            }).collect()
        })
        .unwrap_or_default();

    assert!(dns_sans.contains(&"sub.credo.test"),  "sub.credo.test must be a SAN");
    assert!(dns_sans.contains(&"alt.credo.test"), "alt.credo.test must be a SAN");
}
...
fn sign_csr_rejects_policy_violation() {
    ...
    let result = vigil::issuance_policy::validate_issuance_policy(
        &csr, &[], &config.issuance_policy,
    );
    assert!(result.is_err(), "policy violation must return an error");
    let msg = result.unwrap_err().to_string();
    assert!(msg.contains("evil.com"), "error must mention the disallowed name");
}
