...
use crate::pki_wire::{build_ocsp_error_response, build_ocsp_response_from_request, build_signed_crl_der, build_signed_crl_pem};
...
pub fn build_router(state: AppState) -> Router {
    ...
    let acme_protected = Router::new()
        ...
        .route("/acme/order/:id/finalize",post(crate::acme::finalize_order))
...
async fn ocsp_by_id(
    ...
    let config = state.config();
    let ocsp = get_ocsp_status_by_cert_id(&config.cert_db_path, &id, config.ca.ocsp_max_age_seconds)?;
    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.ocsp.checked", &auth_user.id,
...
async fn ocsp_by_serial(
    ...
    let serial = q.serial_number.filter(|s| !s.trim().is_empty())
        .ok_or_else(|| AppError::BadRequest("serialNumber query parameter is required.".to_string()))?;
    let config = state.config();
    let ocsp = get_ocsp_status_by_serial(&config.cert_db_path, &serial, config.ca.ocsp_max_age_seconds)?;
    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.ocsp.checked", &auth_user.id,
...
async fn ocsp_der(
    ...
    match build_ocsp_response_from_request(&body, config.ca.ocsp_max_age_seconds, &config) {
        Ok(resp) => {
            ...
            (StatusCode::OK, [("Content-Type", "application/ocsp-response")], resp).into_response()
        }
        Err(e) => {
            tracing::warn!("DER OCSP request failed: {}", e);
            (StatusCode::BAD_REQUEST, [("Content-Type", "application/ocsp-response")], build_ocsp_error_response(1)).into_response()
        }
    }
}
...
async fn crl_json(
    ...
    let config = state.config();
    let crl = generate_crl(&config.cert_db_path, state.ca_metadata(), config.ca.crl_next_update_hours)?;
    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.crl.downloaded", &auth_user.id,
...
async fn sign_certificate(
    ...
    let csr_pem = body.csr_pem.filter(|s| !s.trim().is_empty())
        .ok_or_else(|| AppError::BadRequest("csrPem is required.".to_string()))?;

    let days = body.days.map(|d| d as u32).unwrap_or(state.config().ca.cert_default_days);
    if days == 0 {
        return Err(AppError::BadRequest("days must be a positive number.".to_string()));
    }

    let extra_sans: Vec<String> = body.sans.unwrap_or_default().into_iter().filter(|s| !s.trim().is_empty()).collect();
    ...
    let stored = storage::issue_certificate_record(&config.cert_db_path, &config.certs_dir, record, &signed.fullchain_pem)?;

    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.signed", &auth_user.id,
...
async fn get_certificate(
    ...
    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.downloaded", &auth_user.id,
        json!({ "certificateId": id }),
    );
    Ok(Json(json!({ "certificate": { "certPem": cert_pem, "id": record.id, "serialNumber": record.serial_number, "subject": record.subject, "revoked": record.revoked } })))
}
...
async fn revoke_certificate(
    ...
    if auth_user.role != Role::Admin {
        ...
        if owner != auth_user.id && existing.issued_by != auth_user.id {
            return Err(AppError::Forbidden("Certificate does not belong to authenticated user.".to_string()));
        }
    }

    let reason = body.reason.filter(|s| !s.trim().is_empty()).unwrap_or_else(|| "unspecified".to_string());
    let updated = storage::revoke_certificate(&config.cert_db_path, &id, &auth_user.id, &reason, Some(auth_user.id.clone()), None, Some("api-user".to_string()))?
        .ok_or_else(|| AppError::NotFound("Certificate not found.".to_string()))?;

    let _ = crate::ctlog::append_ct_log(
        &config.ct_log_path, "certificate.revoked", &auth_user.id,
